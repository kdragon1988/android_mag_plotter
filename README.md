...
| バージョン | リポジトリ | 状態 |
|-----------|-----------|------|
| **Android（安定版）** | このリポジトリ | ✅ v2.3.1 リリース済み |
...
| 改善項目 | 内容 |
|---------|------|
| **USB磁気センサー対応** | 記録時にUSB磁気センサー（IST8310）の値を正しく使用 |
| **閾値の統一** | ゲージ・ヒートマップ・統計表示の閾値を統一（5/10μT固定） |
| **パフォーマンス最適化** | 差分更新方式で200ポイント以上でもスムーズに動作 |
| **危険値の前面表示** | 赤いポイントが緑に隠れないようZ順序を制御 |

### 📍 現在位置アイコンの改善

従来の指アイコンから**青い丸印**に変更し、正確な位置が一目でわかるようになりました。
...
Raspberry Pi Picoに**RGB LED**と**パッシブブザー**を接続し、ノイズレベルに応じたリアルタイムフィードバックが可能になりました。

| フィードバック | 緑（安全） | 黄（警告） | 赤（危険） |
|--------------|----------|----------|----------|
| **LED色** | 🟢 緑 | 🟡 黄 | 🔴 赤 |
| **ブザー音程** | 低音（440Hz） | 中音（880Hz） | 高音（1760Hz） |
| **ビープ間隔** | 長い（1秒） | 中間（0.5秒） | 短い（0.2秒） |

バリオメーター（高度計）のような直感的なフィードバックで、スマホ画面を見なくても磁場状態を把握できます。
...
| バージョン | リリース日 | 主な変更点 |
|-----------|-----------|-----------|
| v2.3.1 | 2025-01 | ヒートマップ記録改善（USB磁気センサー値対応）、閾値統一、パフォーマンス最適化、危険値前面表示、現在位置アイコン改善 |
| v2.3.0 | 2025-01 | Picoノイズフィードバック機能（RGB LED+ブザー）、起動メロディ（ファミマ音） |
